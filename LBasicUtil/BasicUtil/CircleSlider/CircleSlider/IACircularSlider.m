...
@property (assign, nonatomic, readwrite) IBInspectable CGFloat value;
...
-(void)calculateForTouch:(UITouch*)touch{
    ...
    CGFloat tempValue = [self boundValueForValue:[self valueForRadian:transN]];
    
    int remainder = (int)(tempValue)%self.spaceValue;
    if (remainder!=0) {
        tempValue = [self boundValueForValue:(int)(tempValue)-remainder+self.spaceValue];
        [self updateProgressWithValue:tempValue];
    }else{
